...
class VoiceMoveBase:
    ...
    def __init__(self,_xA=1,_yA=1,_thA=0,_xB=0,_yB=0,_thB=0,_xC=0,_yC=0,_thC=0):
	...
	self._C.target_pose.pose.position.y = _xC
...
#	self.pubManipulation = rospy.Publisher('cmd_Followme', String)
	...
	while not rospy.is_shutdown():
		...
		r.sleep()
    def _Start(self):
	self._VoiceThread = threadingThread(target=self._VoiceState)		
	self._VoiceThread.setDaemon(True)
